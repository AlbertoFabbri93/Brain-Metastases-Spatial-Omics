...
ggsave(paste(patient_subfolder, "Patient_",  patient_num, "_krt17.png", sep = ""))
  ...
  for (cluster in clusters) {
    ...
    patient_rna_filtered <- subset(patient_rna_only, idents = clusters_above_threshold)
    # Find markers (differentially expressed genes) for each of the identity classes in the filtered dataset
    markers.patient_rna_filtered <- FindAllMarkers(patient_rna_filtered, only.pos = TRUE)
    
    # Filter markers to get the most significant ones per cluster
    most_significant_markers <- markers.patient_rna_filtered %>%
      group_by(cluster) %>%
      filter(avg_log2FC > 1) %>%
      slice_head(n = 10) %>%
      ungroup()
    ...
    # Define the size parameter for the cluster heatmap text proportionally to the smallest cluster size
    calculate_heatmap_size <- function(smallest_cluster_size) {
      base_size <- 10  # Base size for the heatmap
      # scaling_factor <- 0.1
      return(base_size * exp(log(smallest_cluster_proportion)/6))
    }

    heatmap_size <- calculate_heatmap_size(smallest_cluster_size)

    # Create the heatmap with the scaled text
    diff_expr_genes_heatmap <- DoHeatmap(
      patient_rna_filtered,
    ...
    ) + ggtitle(paste("Patient", patient_num, cluster), subtitle = "Top 10 Differentially Expressed Genes per Cluster")
    ...
    ggsave(
      filename = paste0(patient_subfolder, "Patient_",  patient_num, "_", cluster, "_diff_expr_genes_heatmap.png"),
    ...
    ggsave(paste(patient_subfolder, "Patient_",  patient_num, "_", cluster,"_umap.png", sep = ""))
    ...
    for(core in unique(patient_data@meta.data$core_serial)) {
      for (stamp in unique(patient_rna_only@meta.data$stamp[patient_rna_only@meta.data$core_serial == core])) {
        ...
        ) + ggtitle(paste("Patient", patient_num, "Core", core, ", Stamp", stamp), subtitle = cluster)
        clustering_plots_list[[paste(cluster, core, as.character(stamp), sep = "_")]] <- stamp_plot
        ggsave(paste(patient_subfolder, "Patient_",  patient_num, "_", cluster, "_core_", core, "_stamp_", stamp, ".png", sep = ""))
      }
    }
  }
