...
ggsave(paste(patient_subfolder, "Patient_",  patient_num, "_krt17.png", sep = ""))
  ...
  markers.patient_rna_only %>%
    ...
    slice_head(n = 10) %>%
    ungroup() -> top10
  top10genes_heatmap <- DoHeatmap(patient_rna_only, features = top10$gene, assay = "Nanostring") + ggtitle(paste("Patient", patient_num), subtitle = "Top 10 Differentially Expressed Genes")
  ggsave(paste(patient_subfolder, "Patient_",  patient_num, "_top_10_genes_heatmap.png", sep = ""))
  ...
  ggsave(paste(patient_subfolder, "Patient_",  patient_num, "_cell_genes_pheatmap.png", sep = ""))
  ...
  InSituType_clusters <- DimPlot(patient_rna_only, reduction = "umap", group.by = "InSituTypeIDs_semisupervised")
  ...
  for (stamp in unique(patient_rna_only@meta.data$stamp)) {
    stamp_plot <- ImageDimPlot(patient_rna_only, fov = patient_image, border.color = NA, cells = row.names(patient_rna_only@meta.data)[which(patient_rna_only@meta.data$stamp == stamp)]) + theme(
  ...
  legend.spacing = unit(0.5, 'lines') # Adjust the spacing between legend items
    ...
    stamps_list[[as.character(stamp)]] <- stamp_plot
    ggsave(paste(patient_subfolder, "Patient_",  patient_num, "_stamp_", stamp, ".png", sep = ""))
  }
  
  # Return all plots together, otherwise only the last one is shown
  plot_list <- c(list(elbow_plot, umap_plot, panCK_plot, KRT17_plot, top10genes_heatmap, cell_genes_pheatmap, InSituType_clusters), stamps_list)
  print(plot_list)
}
```
