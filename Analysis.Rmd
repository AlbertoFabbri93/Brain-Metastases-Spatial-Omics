...
for (patient_id in sort(unique(unlist(breast.cancer.patients$Patient.ID)))) {
  ...
  for (serial_core in serial_cores) {
    # Subset the data for the current serial core
    serial_core_data <- subset(x = patient_data, subset = core_serial == serial_core)
    ...
    for (stamp_num in stamps) {
      ...
      stamp_data <- subset(x = serial_core_data, subset = stamp == stamp_num)
