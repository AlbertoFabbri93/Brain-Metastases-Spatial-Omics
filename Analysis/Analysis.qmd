...
combined_data_filtered_4p <- combined_data %>%
...
cell_types <- unique(combined_data_filtered_4p$`Cells Type`)

# Pre-calculate y-axis limits for each Cell Type
y_limits <- combined_data_filtered_4p %>%
  group_by(`Cells Type`) %>%
  summarise(
...
for (cell_type in cell_types) {
  ...
  for (pair in method_pairs) {
    data_filtered <- combined_data_filtered_4p %>%
...
combined_plots <- plot_grid(plotlist = plot_list, ncol = 3, align = 'hv') + plot_layout(
  heights = unit(c(18,-1),c("cm","null"))  # Increase the height factor
)
compare_patients_to_save[["Wilcoxon_all_patients_scatterplot"]] <- combined_plots
ggsave("Wil.png", path = here())
...
for (cell_type in cell_types) {
  ...
  for (pair in method_pairs) {
    ...
    # Create the paired data object
    pd <- paired(m1_data, m2_data)
    
    # Rename the columns of the paired data object
names(pd) <- pair
    ...
    # Create the profile plot using PairedData's plot function
    ...
    # Add the plot to the list
    plot_list2[[paste(cell_type, pair[1], pair[2], sep = "_")]] <- p
  }
}


# Combine plots using cowplot's plot_grid
combined_plots2 <- plot_grid(plotlist = plot_list2, ncol = 3, align = 'hv') + plot_layout(
  heights = unit(c(18,-1),c("cm","null"))  # Increase the height factor
)
compare_patients_to_save[["Wilcoxon_all_patients_profileplot"]] <- combined_plots2
ggsave("Wil2.png", path = here())
