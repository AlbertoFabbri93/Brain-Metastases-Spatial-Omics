...
IST_ss_clusters_data$Method <- "InSituType"
...
RNA_ann_clusters_data$Method <- "Louvain"
...
combined_stacked_barplot <- ggplot(combined_data,
                           aes(x = `Patient ID`, 
                               y = `Cell Quantity Proportion`, 
                               fill = `Cells Type`, 
                               group = Method)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  facet_wrap(~ Method, ncol = 1) +  # This will separate the methods into separate rows
