...
```{r}
#| label: tidy-seurat-object

# Load the Seurat object
breast_cancer_patients_mixed_features <- readRDS(here(rds_dir, "breast_cancer_patients.rds"))
...
tidy_seurat_object_assay <- function(seurat_object_mixed_features, assay_name) {
  ...
  gene_features <- setdiff(rownames(assay), c(system_control_features, negative_probes_features))
...
breast_cancer_patients_cleaned_features <- tidy_seurat_object_assay(breast_cancer_patients_mixed_features, "Nanostring")
# rm(breast_cancer_patients_mixed_features)
```
...
breast_cancer_patients <- breast_cancer_patients_cleaned_features
...
Insitutype_color_lookup_table <- generate_colors_lookup_table(patient_1_data, "ist_semisup_clusters_ann", known_clusters_colors)
# Generate the annotated Insitutype plots
patient_1_plots <- c(patient_1_plots, generate_clustering_plots(patient_1_data, "ist_semisup_clusters_ann",
    cluster_assay = "RNA", 
