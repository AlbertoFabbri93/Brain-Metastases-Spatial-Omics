...
analyze_patient <- function(patient_num) {
  ...
  for (cluster in clusters) {
    ...
    ggsave(
      filename = paste0(patient_subfolder, "Patient_",  patient_num, "_", cluster,"_umap", image_ext),
      plot = umap_clusters
    )
    ...
    for(core in unique(patient_rna_only@meta.data$core_serial)) {
      for (stamp in unique(patient_rna_only@meta.data$stamp[patient_rna_only@meta.data$core_serial == core])) {
        ...
        ggsave(
          filename = paste0(patient_subfolder, "Patient_",  patient_num, "_", cluster, "_core_", core, "_stamp_", stamp, image_ext),
          plot = stamp_plot)
      }
    }
  }
  ...
  contingency_tab_clusters <- table(
    patient_rna_only$InSituType_semisup_clusters,
    patient_rna_only$seurat_clusters,
  ...
  ggsave(
    filename = paste0(patient_subfolder, "Patient_",  patient_num, "_heatmap_seurat_vs_insitutype", image_ext),
    plot = heatmap_seurat_vs_insitutype
  )

  # Return all plots together, otherwise only the last one is shown
  plot_list <- c(list(elbow_plot, panCK_plot, KRT17_plot, heatmap_seurat_vs_insitutype), clustering_plots_list)
  print(plot_list)
}
```
